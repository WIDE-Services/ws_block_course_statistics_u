...
use block_course_statistics\output\measurespertool;
use block_course_statistics\output\measuresusertool;
use block_course_statistics\output\measurescoursetool;
...
$viewmodules = optional_param('viewmodules', 0, PARAM_INT);
$viewusers = optional_param('viewusers', 0, PARAM_INT);
$cminstance = optional_param('cminstance', 0, PARAM_INT);
...
if ($viewusers != 0 ) {
    ...
    // Detailed Results for all enrolled users in a course modules (activities) .

    // Retrieve the scheduled precalculated data of users in an course activity.

    if ($isteacher) {

        $measures = $logic->group_viewusers_data($courseid , $cminstance , $searchperiod , $from , $to);

        $maindashboard = new measuresusertool($params , $data->form , $measures["usersdata"] ,
        ...
        $maindashboard = new measuresusertool(null , null , null ,
                null , null , null , $access = false);

    }


} else if ($viewmodules != 0) {
    ...
    if ($isteacher) {
        ...
        $measures = $logic->group_activities_measures_data($courseid , $searchperiod , $from , $to);

        $maindashboard = new measurescoursetool($params , $data->form , $measures["activitiesdata"] ,
                $searchperiod , $from , $to , $access = true);
        ...
        $maindashboard = new measurescoursetool(null , null , null ,
                null , null , null , $access = false);
    }
    ...
    if ($isteacher) {
        ...
        $measures = $logic->group_courses_tools_data($courseid , $isteacher , $searchperiod , $from , $to);

        $maindashboard = new measurespertool($params , $data->form , $measures["generaldata"] ,
                $searchperiod , $from , $to , $access = true);
        ...
        $maindashboard = new measurespertool(null , null , null ,
                null , null , null , $access = false);
